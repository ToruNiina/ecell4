...
SingleConicalPropagator::escape(const SingleConicalDomain& dom)
{
    ...
    const auto new_pos = ecell4::polygon::roll(world_.polygon(),
            ...
            dom.vertex_id(), dom.effective_radius(), theta);
...
boost::container::static_vector<ParticleID, 2>
SingleConicalPropagator::reaction(const SingleConicalDomain& dom)
{
    ECELL4_NGFRD_LOG_FUNCTION();
...
boost::container::static_vector<ParticleID, 2>
SingleConicalPropagator::attempt_1to1_reaction(const SingleConicalDomain& dom,
...
boost::container::static_vector<ParticleID, 2>
SingleConicalPropagator::attempt_1to2_reaction(const SingleConicalDomain& dom,
        const ParticleID& pid, const Particle& p, const FaceID& fid,
        const ReactionRule& rule)
{
    ...
    if(D1 == 0 && D2 == 0)
    {
        throw_exception<NotSupported>("ngfrd::SingleConicalPropagator attempts"
...
//
ParticleID SingleConicalPropagator::propagate(
        const SingleConicalDomain& dom, const Real dt)
{
    ECELL4_NGFRD_LOG_FUNCTION();

    const auto pid = dom.particle_id();
    const auto vid = dom.vertex_id();
...
ReactionRule const& SingleConicalPropagator::determine_reaction_rule(
...
bool SingleConicalPropagator::is_inside_of_shell(const SingleConicalDomain& dom,
        const std::pair<Real3, FaceID>& pos, const Real radius)
{
    if(dom.shell().thickness() < radius) // in the 3D direction
    {
        return false;
    }
    const auto vid = dom.vertex_id();
    const Real dist = ecell4::polygon::distance(world_.polygon(), pos,
            std::make_pair(dom.shell().position(), vid));

    return dist + radius < dom.shell().shape().slant_height();
}

Real3 SingleConicalPropagator::draw_2D_displacement(const FaceID& fid, const Real len)
{
